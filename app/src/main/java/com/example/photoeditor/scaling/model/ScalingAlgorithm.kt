...
class ScalingAlgorithm {
    ...
    private fun resizeBitmap(source: Bitmap, k: Double): Bitmap {
        if (k < 0.01) {
            //Toast.makeText(this, "Minimum value 0.01", Toast.LENGTH_SHORT).show()
            return source
        }
        ...
        try {
            if (source.height >= source.width) {
                ...
                //Toast.makeText(this, "The image is scaled", Toast.LENGTH_SHORT).show()
                return result
            }
        } catch (e: Exception) {
            return source
    ...
    public fun bilinearInterpolation(bitmap: Bitmap, k: Double): Bitmap {
        if (bitmap.height * bitmap.width * k * k >= 26214400) {
            //Toast.makeText(this, "Enter a lower coefficient", Toast.LENGTH_SHORT).show()
            return bitmap
        }
